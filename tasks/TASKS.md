...
#### Phase 1: Core Client
- [x] [TASK-198] Create `sdk/python/slancha/models.py` — Pydantic v2 models: ChatCompletion, ChatCompletionChunk, Choice, Message, Usage, EvalRun, EvalResult, FineTuneJob, Deployment, Provider, RouterModel, RoutingDecision. Match OpenAI response schema for chat completions. Update __init__.py exports. · priority:high · owner:claude-code · project:slancha · created:2026-04-01 · done:2026-04-01
- [x] [TASK-199] Create `sdk/python/slancha/client.py` — Slancha sync client using httpx. Constructor takes api_key + optional base_url (default https://api.slancha.ai/v1). Implement chat.completions.create(model, messages, **kwargs) → POST /v1/chat/completions. Parse into ChatCompletion. Errors: 401→AuthenticationError, 429→RateLimitError, 5xx→APIError, timeout→TimeoutError. Support streaming via stream=True returning iterator of ChatCompletionChunk. · priority:high · owner:claude-code · project:slancha · created:2026-04-01 · done:2026-04-01
- [x] [TASK-200] Create `sdk/python/slancha/async_client.py` — AsyncSlancha using httpx.AsyncClient. Same interface but async. Support async streaming. Re-export from __init__.py. · priority:high · owner:claude-code · project:slancha · created:2026-04-01 · done:2026-04-01

#### Phase 2: Management Stubs & Utilities
- [x] [TASK-201] Add management method stubs — client.providers.list/create/delete/toggle, client.models.list/create/delete, client.routing.decisions.list/create/update/delete, client.config.generate_yaml/save_snapshot. Each raises NotImplementedError("Management API coming soon — use the dashboard at slancha.ai/dashboard"). · priority:med · owner:claude-code · project:slancha · created:2026-04-01 · done:2026-04-01
- [x] [TASK-202] Add retry logic — exponential backoff with jitter on 429/5xx. Default max_retries=3. Respect Retry-After header. Configurable timeout (30s connect, 120s read). · priority:med · owner:claude-code · project:slancha · created:2026-04-01 · done:2026-04-01

#### Phase 3: Tests & Packaging
- [x] [TASK-203] Write pytest tests — test chat.completions.create (mock httpx via respx), streaming, error handling (401/429/5xx/timeout), async client, pydantic model parsing, retry logic. 39 tests in `sdk/python/tests/`. · priority:high · owner:claude-code · project:slancha · created:2026-04-01 · done:2026-04-01
- [x] [TASK-204] Write SDK README.md — pip install, quickstart (3 lines), streaming, async, error handling, config options. Concise, developer-focused. · priority:med · owner:claude-code · project:slancha · created:2026-04-01 · done:2026-04-01
- [x] [TASK-205] Finalize packaging — verify pyproject.toml, add py.typed, run pytest (39/39 passing), verify all passes. · priority:med · owner:claude-code · project:slancha · created:2026-04-01 · done:2026-04-01
...
### Sprint 14: Pilot Readiness & Infrastructure (2026-04-01)

**CONTEXT:** Copywriter, strategist, finance, ops, bizdev, designer, QA all have ZERO pending tasks. Queue has 2 tasks total (both for claude-code, who is BROKEN with 42 consecutive errors). Need to generate work immediately for idle agents. Focus on marketing site gaps (docs section, blog section) and enterprise requirements (data isolation architecture).

- [ ] [TASK-206] Build /docs section on marketing site — create /docs route with sidebar navigation (Quickstart, API Reference, Integration Guides, Architecture, Webhooks, Glossary). Use existing copy from copywriter docs (api-reference-complete.md, quickstart.md, architecture-overview.md). Add to Nav dropdown, footer, sitemap. Lazy load with React Router. Follow existing docs page patterns from Ralph's work. Output: /docs route with all content, working nav, proper meta tags · priority:high · owner:frontend · project:slancha · created:2026-04-01
- [ ] [TASK-207] Build /blog section on marketing site — create /blog route with grid layout (23 blog posts), individual blog post pages with TOC, related posts, CTA, OG tags, JSON-LD schema. Add to Nav dropdown, footer, sitemap. Use Ralph's existing blog posts from site/blog/. Follow blog patterns from Ralph's iteration 80+ work. Output: /blog route with all posts, working post navigation, proper SEO · priority:high · owner:frontend · project:slancha · created:2026-04-01

#### Phase: Social Media & Content Distribution (INIT-05)
- [ ] [TASK-210] Write social media content templates — create 20 LinkedIn posts, 10 Twitter/X threads, 5 newsletter snippets based on existing blog posts. Each post should have: headline hook, body copy (2-3 paragraphs), CTA, hashtag set. Output to `workspace-copywriter/docs/social-media-templates.md` · priority:med · owner:copywriter · project:slancha · created:2026-04-01

#### Phase: Sales Enablement (INIT-07)
- [ ] [TASK-211] Create 3 vertical case study templates — fintech, healthtech, e-commerce templates with placeholders for metrics, quotes, implementation timeline, compliance badges. Each template: 1-page PDF layout with narrative structure, ROI calculation table, before/after comparison. Output to `workspace-copywriter/docs/case-study-templates.md` · priority:med · owner:copywriter · project:slancha · created:2026-04-01
...
- [ ] [TASK-214] Create agent health monitoring process — define metrics to track (error rates, response times, task completion rates), alerting thresholds, weekly health report format. Output to `workspace-ops/processes/agent-health-monitoring.md` · priority:med · owner:ops · project:slancha · created:2026-04-01
...
- [x] [TASK-209] Write Model Data Isolation architecture doc — technical deep dive for enterprise security reviews: single-tenant vs multi-tenant, model weight isolation, eval data encryption at rest/in transit, API key vault integration (Supabase vault RPCs), network isolation (VPC peering), audit logging for data access, customer verification tools. Output to workspace-ops/docs/data-isolation.md · priority:high · owner:ops · project:slancha · created:2026-04-01 · started:2026-04-01 · done:2026-04-01
...
- [x] [TASK-192] Write blog post: "Why eval data should drive deployment decisions" — 2,500-word technical deep dive on how eval metrics should inform model selection, deployment strategy, and when to fine-tune vs. route. Include code examples comparing decisions with/without eval data. Target keyword: "eval-driven deployment" · priority:med · owner:copywriter · project:slancha · created:2026-03-31 · started:2026-04-01 · done:2026-04-01
- [x] [TASK-193] Write blog post: "The hidden cost of not post-training" — business case for continuous improvement loop. ROI framework: calculate wasted spend on unoptimized models (typical 30-50% overspend). Include customer case study template. Target keyword: "AI inference cost optimization" · priority:med · owner:copywriter · project:slancha · created:2026-03-31 · started:2026-04-01 · done:2026-04-01
